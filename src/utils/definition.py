...
class Item(TypedDict):
    ...
    count: int
    sprite_path: str
    option: int  # 1=Pokeball, 2=Potion, 6=Shield, 8=Magic Hand

def get_menu_sprite_path(sprite_index: int) -> str:
    """
    Convert sprite index to menu_sprites path.
    sprite_index: 1-16 (or higher, will wrap around)
    Returns: "menu_sprites/menusprite{N}.png"
    
    Example:
        get_menu_sprite_path(1) -> "menu_sprites/menusprite1.png"
        get_menu_sprite_path(3) -> "menu_sprites/menusprite3.png"
    """
    # Clamp or wrap index to available menu sprites (1-16)
    clamped = ((sprite_index - 1) % 16) + 1
    return f"menu_sprites/menusprite{clamped}.png"